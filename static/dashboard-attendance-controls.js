...
const refreshUpdateLabel=()=>document.querySelectorAll('.important-block b').forEach(x=>{if(x.textContent.trim()==='Important dates')x.textContent='Important updates'});refreshUpdateLabel();
...
async function syncAutomaticAttendanceV21(date, force = false) {
  ...
  try {
    ...
    const mappedAttendance = payload.attendance.map(item => {
      ...
      login: automaticTimeV21(item.check_in_at), logout: item.check_out_at ? automaticTimeV21(item.check_out_at) : '',
      work: item.check_out_at ? durationTextV21(item.worked_minutes || 0) : 'In progress', half: '-', overtime: '-', total: item.check_out_at ? durationTextV21(item.worked_minutes || 0) : 'In progress',
