...
landing.className='msme-landing';
landing.innerHTML=`<div class="landing-copy"><div class="landing-logo"><svg viewBox="0 0 120 76" role="img" aria-label="MSME M logo"><path d="M12 61 35 15"/><path d="M35 15 59 61"/><path d="M59 61 83 15"/><path d="M83 15 108 61"/></svg><span><b>MSME</b><small>Track · Manage · Grow</small></span></div><div class="landing-title"><h1>MSME<br><i>MULTI SKILL</i><br>PLANNER</h1><em></em><p>A smart workforce management system for Indian MSMEs to plan, manage and optimize employee skills, attendance and efficiency.</p></div><div class="landing-benefits"><span><b>♙</b>Manage<br>Workforce</span><span><b>▥</b>Track<br>Attendance</span><span><b>⚙</b>Grow<br>Together</span></div><button type="button" class="landing-enter">Go to Workspace <span>→</span></button></div><div class="landing-visual" aria-hidden="true"><div class="landing-glass landing-people">♟♟</div><div class="landing-glass landing-calendar">▦</div><div class="landing-glass landing-chart">↗<i></i><i></i><i></i><i></i></div><div class="landing-glass landing-check">✓</div><div class="landing-desk"></div><div class="landing-laptop"><div><svg viewBox="0 0 120 76"><path d="M12 61 35 15"/><path d="M35 15 59 61"/><path d="M59 61 83 15"/><path d="M83 15 108 61"/></svg></div><i></i></div><div class="landing-pot"><i></i><i></i><i></i><b></b></div><div class="landing-mug">Ideas<br>Build<br>Business</div><div class="landing-tag">PEOPLE &nbsp; | &nbsp; PRODUCTIVITY &nbsp; | &nbsp; PROGRESS</div></div>`;
screen.parentNode.insertBefore(landing,screen);
...
landing.querySelector('.landing-enter').addEventListener('click',()=>{landing.classList.add('hidden');screen.classList.remove('entry-hidden')});
screen.innerHTML=`<div class="login-art"><div class="os-logo"><svg class="msme-mark" viewBox="0 0 120 76" role="img" aria-label="MSME M logo"><defs><linearGradient id="msme-pink" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#ff49cf"/><stop offset=".58" stop-color="#d22df0"/><stop offset="1" stop-color="#6848ff"/></linearGradient><linearGradient id="msme-fold" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#5b2be5"/><stop offset=".52" stop-color="#253eea"/><stop offset="1" stop-color="#087fff"/></linearGradient><linearGradient id="msme-cyan" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#18d4e8"/><stop offset=".52" stop-color="#0a9ef2"/><stop offset="1" stop-color="#4350ff"/></linearGradient><linearGradient id="msme-blue" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="#165df7"/><stop offset="1" stop-color="#4f3dff"/></linearGradient></defs><path d="M12 61 35 15" fill="none" stroke="url(#msme-pink)" stroke-width="19" stroke-linecap="round"/><path d="M35 15 59 61" fill="none" stroke="url(#msme-fold)" stroke-width="19" stroke-linecap="round"/><path d="M59 61 83 15" fill="none" stroke="url(#msme-cyan)" stroke-width="19" stroke-linecap="round"/><path d="M83 15 108 61" fill="none" stroke="url(#msme-blue)" stroke-width="19" stroke-linecap="round"/></svg><span class="msme-logo-copy"><b>MSME</b><small>Track · Manage · Grow</small></span></div><div class="art-copy"><h2>MSME<br><i>MULTI SKILL</i><br><i>PLANNER</i></h2><span></span><p>A smart workforce management system for Indian MSMEs to plan, manage and optimize employee skills, attendance and efficiency.</p></div><div class="login-benefits"><span><b>♙</b>Manage<br>Workforce</span><span><b>◷</b>Track<br>Attendance</span><span><b>✦</b>Grow<br>Together</span></div></div><div class="login-side"><div class="login-card"><button type="button" class="back-to-welcome">← Back to Welcome</button><p class="eyebrow">WELCOME BACK</p><h1>Welcome to your workspace</h1><p class="login-copy">Sign in or create a private MSME account.</p><div class="access-switch"><button data-access="workspace" class="active">Workspace - Admin</button><button data-access="worker">Workspace - E / S / W / TW</button></div><div id="owned-auth-panel"></div><div class="auth-tabs"><button data-tab="signin" class="active">← Back to sign in</button><button data-tab="signup">Create profile</button></div></div><div class="login-scene" aria-hidden="true"><div class="scene-user">●</div><div class="scene-checklist"><i></i><i></i><i></i></div><div class="scene-plant"><i></i><i></i><i></i><b></b></div><div class="scene-shelf"><span></span><span></span><span></span></div><div class="scene-laptop"><div class="laptop-screen"><svg viewBox="0 0 120 76"><path d="M12 61 35 15"/><path d="M35 15 59 61"/><path d="M59 61 83 15"/><path d="M83 15 108 61"/></svg></div><div class="laptop-base"></div></div><div class="scene-plaque">SKILLS<br>PEOPLE<br>GROWTH</div></div></div>`;
...
screen.querySelector('.back-to-welcome').addEventListener('click',()=>{screen.classList.add('entry-hidden');landing.classList.remove('hidden')});
...
function mailIcon(){return `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3.5 6.5h17v11h-17z"/><path d="m4.5 7.5 7.5 6 7.5-6"/></svg>`}
...
function eye(){return `<button type="button" data-eye aria-label="Show password"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2.5 12s3.5-6 9.5-6 9.5 6 9.5 6-3.5 6-9.5 6-9.5-6-9.5-6Z"/><circle cx="12" cy="12" r="2.5"/></svg></button>`}
...
function bindEmployeeAccess(){
 ...
 if(activation){
  ...
  activation.onsubmit=async event=>{event.preventDefault();normalizeEmployeeInvitation(activation);const fields=Object.fromEntries(new FormData(activation)),button=activation.querySelector('button[type="submit"]');if(!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(String(fields.contact||'').trim()))return message('Enter the email address used for this invitation.',true);button.disabled=true;button.textContent='Creating account…';try{const created=await employeeRequest('/api/employee/activate',fields);sessionStorage.setItem('msme-employee-token',created.access_token);alert('Account created successfully.');await window.MSMEEmployeePortal?.open()}catch(error){message(error.message,true);button.disabled=false;button.textContent='Create account'}};
 }
}
...
function show(){
 if(mode==='worker'){
  if(tab==='signin')panel.innerHTML=`<form class="auth-form" id="owned-employee-signin"><h2>Workforce sign in</h2><p>Workers, Staff, Temporary Workers and Entrepreneurs use credentials issued by their administrator.</p><label>PHONE NUMBER / EMAIL<span class="input-icon-wrap">${mailIcon()}<input name="email" type="text" required autocomplete="username" placeholder="Enter registered email"></span></label><label>PASSWORD<span class="password-wrap input-icon-wrap">${lockIcon()}<input name="password" type="password" required autocomplete="current-password">${eye()}</span></label><button class="forgot" type="button" data-employee-forgot>Forgot password?</button><p data-message></p><button class="primary" type="submit">Sign in</button></form>`;
  else {const query=employeeInviteParams();panel.innerHTML=`<form class="auth-form" id="owned-employee-activate"><h2>Create employee account</h2><p>Open the invitation link to fill this form automatically, or paste the copied invitation link/token manually below.</p><label>INVITATION LINK OR TOKEN<input name="invite_token" value="${esc(query.get('employee_invite')||'')}" required autocomplete="off" placeholder="Paste the complete invitation link or token"></label><label>EMAIL ADDRESS<input name="contact" type="email" value="${esc(query.get('employee_email')||'')}" required placeholder="Enter the email address on your invitation"></label><label>CREATE PASSWORD<span class="password-wrap"><input name="password" type="password" minlength="8" required>${eye()}</span></label><p data-message></p><button class="primary" type="submit">Create account</button></form>`;}
  bindEmployeeAccess();panel.querySelector('[data-employee-forgot]')?.addEventListener('click',employeeForgotPassword);return
 }
 if(tab==='signin'){const remembered=localStorage.getItem('msme-last-login-email')||'';panel.innerHTML=`<form class="auth-form" id="owned-signin"><label>EMAIL<span class="input-icon-wrap">${mailIcon()}<input type="email" name="email" value="${esc(remembered)}" required autocomplete="username"></span></label><label>PASSWORD<span class="password-wrap input-icon-wrap">${lockIcon()}<input type="password" name="password" required autocomplete="current-password">${eye()}</span></label><button type="button" class="forgot" data-forgot>Forgot password?</button><p data-message></p><button type="button" class="primary" id="owned-signin-submit">Sign in</button></form>`;bind();return}
