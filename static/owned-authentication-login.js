...
function show(){
 if(mode==='worker'){
  ...
  else panel.innerHTML=`<form class="auth-form" id="owned-employee-activate"><h2>Activate employee account</h2><p>Use the invitation supplied by your administrator.</p><label>INVITATION TOKEN<input name="invite_token" required autocomplete="off"></label><label>PHONE NUMBER / EMAIL<input name="contact" required placeholder="Enter the phone number or email on your invitation"></label><label>CREATE PASSWORD<span class="password-wrap"><input name="password" type="password" minlength="8" required>${eye()}</span></label><label>CREATE ATTENDANCE PIN<input name="pin" inputmode="numeric" pattern="[0-9]{6}" maxlength="6" required placeholder="6-digit PIN"></label><p data-message></p><button class="primary" type="submit">Activate account</button></form>`;
